  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `You are an OCR assistant for an educational tutoring platform. Extract the problem or question from the image. ${subjectHint} ${langHint}Return JSON: { "extractedText": "...", "confidence": 0.0-1.0, "detectedSubject": "PHYSICS|CHEMISTRY|..." }`,
        },
        {
          role: 'user',
          content: [
